...
import { effectiveBookingPriceCents } from "@/lib/booking/booking-price";
import { todayYyyyMmDdSaoPaulo } from "@/lib/datetime/sao-paulo-calendar";
import {
...
export default async function DashboardPage({
  ...
  const uid = user.id;
  const todayYmd = todayYyyyMmDdSaoPaulo();
  ...
  const proRowRes = await supabase
    .from("professionals")
    .select("id, slug, rating_avg, reviews_count")
    .eq("id", uid)
    .maybeSingle();
  ...
  const [
    ...
    fetchIncomingBookingsForProOnCalendarDay(uid, todayYmd),
  ]);
  ...
  const proSlug = typeof proRow?.slug === "string" ? proRow.slug.trim() : "";
  const publicProfileUrl = proSlug ? `${getSiteUrl()}/profissional/${proSlug}` : "";

  const todayAgendaCount = todayAgendaRows.length;
  const todayOpenCount = todayAgendaRows.filter(
    (b) => !["completed", "cancelled", "archived"].includes(b.status),
  ).length;
  const todayCompletedCount = todayAgendaRows.filter((b) => b.status === "completed").length;
  const todayCancelledCount = todayAgendaRows.filter((b) => b.status === "cancelled").length;
  const todayCompletedSumCents = todayAgendaRows
    .filter((b) => b.status === "completed")
    .reduce((sum, b) => {
      const c = effectiveBookingPriceCents(b.final_price_cents, b.service_price_cents_snapshot);
        ...
        <div className="dash-body">
          ...
          <div className="dash-content">
            ...
            {publicProfileUrl && proSlug ? (
              <ProfessionalProfileSharePanel profileUrl={publicProfileUrl} slug={proSlug} variant="dashboard" />
            ) : null}
            ...
            <div className="dash-card" style={{ marginBottom: 18 }}>
              ...
              <p style={{ margin: 0, color: "var(--ink50)", fontSize: 14, lineHeight: 1.6, fontWeight: 500 }}>
                {todayAgendaCount === 0 ? (
                  <>Nada agendado para hoje no calendário. Quando um cliente marcar, o horário aparece aqui e na agenda.</>
                ) : (
                  <>
                    <strong>{todayAgendaCount}</strong> compromisso(s) hoje
                    {todayOpenCount > 0 ? (
                      <>
                        {" "}
                        · <strong>{todayOpenCount}</strong> em aberto (pendente, confirmado ou em andamento)
                      </>
                    ) : null}
                    {todayCompletedCount > 0 ? (
                      <>
                        {" "}
                        · <strong>{todayCompletedCount}</strong> concluído(s)
                      </>
                    ) : null}
                    {todayCompletedSumCents > 0 ? (
                      <>
                        {" "}
                        · soma dos concluídos com preço: <strong>{formatEarningsBRL(todayCompletedSumCents)}</strong>
                      </>
                    ) : null}
                    {todayCancelledCount > 0 ? (
                      <>
                        {" "}
                        · <strong>{todayCancelledCount}</strong> cancelado(s)
                      </>
                    ) : null}
                    .
                  </>
