...
import { isServiceAttendanceMode, labelAttendanceShort } from "@/lib/services/service-attendance";
import {
...
export default async function DashboardServicosPage() {
  ...
  const { data: svcRaw } = isPro
    ? await supabase
        .from("pro_services")
        .select(
          "id, name, description, price_cents, duration_minutes, status, reviewed_at, reviewer_note, created_at, category_key, attendance_mode",
        )
          ...
          {!isPro ? (
            ...
            <>
              <p className="sec-sub" style={{ margin: "0 0 18px", maxWidth: 720 }}>
                Cada serviço fica ligado a <strong>uma área</strong> (como na busca) e a <strong>forma de atendimento</strong>{" "}
                (no seu espaço, deslocamento até o cliente ou os dois). O <strong>preço é obrigatório</strong> (valor fixo do
                anúncio) e a <strong>duração</strong> define quanto tempo o horário fica bloqueado na sua agenda. Enquanto
                estiver <strong>em análise</strong>, só você vê aqui.
              </p>
              ...
              {svc.length ? (
                <div className="kz-svc-preview-list">
                  {svc.map((s) => {
                    ...
                    const att = labelAttendanceShort(
                      typeof s.attendance_mode === "string" && isServiceAttendanceMode(s.attendance_mode)
                        ? s.attendance_mode
                        : "at_venue",
                    );
                    ...
                    return (
                      <article key={s.id} className="kz-svc-preview-card">
                        ...
                        <div className="kz-svc-preview-card__price">
                          ...
                          {att ? (
                            <span style={{ fontSize: 12, fontWeight: 700, color: "var(--lime-deep)", marginLeft: 8 }}>
                              · {att}
                            </span>
                          ) : null}
                        </div>
