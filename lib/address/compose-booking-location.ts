...
/** Texto gravado em `client_location` quando o atendimento é só no espaço do profissional. */
export const BOOKING_SNAPSHOT_ON_PRO_VENUE =
  "Atendimento no espaço do profissional (salão, estúdio ou consultório). Endereço exato costuma ser combinado após a confirmação do pedido.";

export function composeProVenueBookingSnapshot(contactPhone: string): string {
  const p = contactPhone.trim();
  if (!p) return BOOKING_SNAPSHOT_ON_PRO_VENUE;
  return `${BOOKING_SNAPSHOT_ON_PRO_VENUE}\nContato: ${p}`;
}
