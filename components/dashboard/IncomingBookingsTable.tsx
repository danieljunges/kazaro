...
function statusStyle(status: string): { background: string; color: string } {
  switch (status) {
    ...
    case "in_progress":
      return { background: "rgba(15, 118, 110, 0.1)", color: "var(--lime-deep)" };
    case "pending":
      ...
      <table className="orders-table">
        ...
        <tbody>
          {rows.map((row) => {
            ...
            const priceCents = effectiveBookingPriceCents(row.final_price_cents, row.service_price_cents_snapshot);
            return (
              <tr
                key={row.id}
              ...
              >
                ...
                <td style={{ textAlign: "right", fontSize: 13, whiteSpace: "nowrap", fontVariantNumeric: "tabular-nums" }}>
                  {formatPriceBRLFromCents(priceCents)}
                </td>
